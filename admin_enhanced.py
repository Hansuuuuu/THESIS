...
class BackupConfigDialog(QDialog):
    def __init__(self, parent=None, selected_clients=None):
        ...
        # Client info - FIXED: Show actual client names
...
class RestoreConfigDialog(QDialog):
    ...
    def _scan_backups(self):
        ...
        # Match with selected clients - FIXED: Show detailed match info
...
class ClientHandler:
    ...
    def send_file_resumable(self, filepath, destination=None):
        ...
        # CRITICAL FIX: Give the reader loop time to see the transferring flag and stop
        # This prevents race condition where reader loop consumes READY/CHUNK_OK messages
        time.sleep(0.15)  # Slightly more than reader loop's 0.1s check interval
            ...
            with self.lock:
                ...
                while not ready_received:
                    ...
                    while b'\n' in buffer:
                        ...
                        # FIXED: More robust READY check with detailed logging
                        msg_upper = msg.upper()
                            ...
                            if len(chunks_in_batch) >= BATCH_ACK_SIZE or idx == len(pending_chunks) - 1:
                                ...
                                while not ack_received and (time.time() - ack_start) < 30:
                                    ...
                                    while b'\n' in ack_buffer:
                                        ...
                                        # FIXED: More robust CHUNK_OK check
    ...
    def _process_backup_data(self):
        try:
            # FIXED: Get clean hostname and sanitize for file system
            import re
            hostname = self.client_info.get("hostname", self.key.replace(":", "_"))
            hostname = re.sub(r'[<>:"/\\|?*]', '_', hostname)  # Remove invalid filename characters
